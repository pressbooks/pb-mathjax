...
function handleError(res) {
  ...
  return res
    .status(400)
    .sendFile(path.resolve("public/images/formula_does_not_parse.png"));
}
...
module.exports.generate = async (configs, req, res, next) => {
  ...
  // Configure TeX input
  let tex = new TeX({
    packages: configs.typeset.math.includes("\\require{physics}")
      ? AllPackages.concat(["physics"])
      : AllPackages,
    inlineMath: [
      ["$", "$"],
      ["\\(", "\\)"],
      ...
      if (isSvg) {
        ...
        try {
          ...
          if (!fullSvgContent.includes('xmlns="http://www.w3.org/2000/svg"')) {
            fullSvgContent = fullSvgContent.replace(
              "<svg",
